...
import { r as scenes, t as groupLabel } from "./scenes-CvkSU-wx.js";
import { i as FEISHU_WEBHOOK_URL, t as getVoteCopy } from "./vote-copy-qImeIajW.js";
//#region app/vote-channel-config.ts
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
/**
* /cvote 渠道版投票的全部可配置项。
*
* 和 /vote（内部版）的关系：同一套场景卡、同一套滑卡交互、同一个飞书 webhook，
* 不同的是——
*   · 客户不用飞书：不做 OAuth 身份验证，直接以「渠道客户」身份提交；
*   · 岗位口径不同：角色选项换成渠道公司的岗位；
*   · 存储键全部独立：同一台设备先后打开 /vote 和 /cvote 互不串进度。
*
* webhook 地址和 TOP_PICKS 与内部版保持一致，直接从 vote-config 取，避免两处
* 各改各的将来对不上。
*/
/**
* 渠道公司（运营商子渠道）的岗位口径。label 会原样写进多维表格的「角色」列，
* 和内部版的岗位标签不重叠，后期筛选/分组时一眼可分。
*/
var CHANNEL_ROLES = [
	{
		id: "channel",
...
/**
* 提交时写进 identity_status 的值。payload 的 key 和内部版完全一致（缺 key 会让
* 飞书自动化报错），feishu_auth_code 固定空串——这条匿名路径内部版本来就有
* （「匿名提交」兜底），自动化已经能处理。
*/
var CHANNEL_IDENTITY_STATUS = "渠道客户";
/** localStorage 键全部独立命名，和内部版互不干扰。 */
var CHANNEL_STORAGE_KEY = "a100.cvote.v1";
var CHANNEL_RECEIPT_KEY = "a100.cvote.receipt.v1";
var CHANNEL_INTRO_SEEN_KEY = "a100.cvote.intro.seen.v1";
//#endregion
//#region app/components/channel/vote-store.ts
/**
...
var roleLabel = (id) => CHANNEL_ROLES.find((r) => r.id === id)?.label ?? "";
/**
...
function saveState(state) {
	try {
		if (state === null) window.localStorage.removeItem(CHANNEL_STORAGE_KEY);
		else window.localStorage.setItem(CHANNEL_STORAGE_KEY, JSON.stringify(state));
	} catch {}
}
/**
* 服务端快照：原始卡序、进度为零的一局。
...
//#endregion
//#region app/components/channel/vote-submit.ts
/**
...
*/
/** 渠道版没有本地模式：webhook 地址编译期就写死，永远发真请求。 */
var isLocalMode = () => false;
function buildWirePayload(payload, identity) {
	if (!identity) return payload;
	return {
		...payload,
		identity_status: CHANNEL_IDENTITY_STATUS,
		feishu_auth_code: ""
	};
}
...
function saveReceipt(payload, result, identity) {
	try {
		window.localStorage.setItem(CHANNEL_RECEIPT_KEY, JSON.stringify({
			at: Date.now(),
			result,
			payload,
...
//#endregion
//#region app/components/channel/vote-result.tsx
...
function VoteResult({ state, onUndo, onRestart }) {
	...
	const send = async () => {
		if (!state.role) {
			setRoleError("请选择岗位后提交");
			firstRoleRef.current?.focus();
			return;
		}
		setRoleError("");
		await finishSend({ status: "channel" });
	};
	const copy = async () => {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("fieldset", {
						...
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("legend", {
								id: "vote-role-legend",
								children: ["你的岗位", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: "必选，用来分组看分歧" })]
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "vote-role-grid",
								children: CHANNEL_ROLES.map((item, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
					...
					result === null ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
						type: "button",
						className: "vote-send",
						onClick: send,
						disabled: sending,
...
//#endregion
//#region app/components/channel/vote-app.tsx
...
function ChannelVoteApp() {
	...
	const showIntro = (0, import_react.useMemo)(() => {
		...
		try {
			return window.sessionStorage.getItem("a100.cvote.intro.seen.v1") !== "1";
		} catch {
			return true;
		}
	...
	(0, import_react.useEffect)(() => {
		if (!showIntro) return;
		try {
			window.sessionStorage.setItem(CHANNEL_INTRO_SEEN_KEY, "1");
		} catch {}
